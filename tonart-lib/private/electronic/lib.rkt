...
(define-art-object (channel [num]))
;; context-free
(define-art-object (full-midi [num velocity instruments channel]))

(define-mapping-rewriter (midi->full-midi [(: m midi)])
  (λ (stx m)
    (syntax-parse m
      [(_ note)
       #:with ((_ inst-name) ...) (context-ref*/surrounding (lookup-ctxt) (get-id-ctxt m) #'instrument)
       #:with (_ channel) (or (context-ref/surrounding (lookup-ctxt) (get-id-ctxt m) #'channel) #'(_ #f))
       (qq-art m (full-midi note 80 [inst-name ...] channel))])))
